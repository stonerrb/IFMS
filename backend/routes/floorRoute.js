...
router.patch("/floors/:floorId/rooms/:roomId", async (req, res) => {
  ...
  router.get('/floors/:floorId/history', async (req, res) => {
    ...
    try {
      ...
      const commitHistory = await commitSchema.find({ id: floorId }).sort({ timestamp: -1 });
      ...
      const floorHistory = await Promise.all(commitHistory.map(async (commit) => {
        // Get the floor details at each commit point
        const floor = await floorModel.find({id:commit.id}).populate('rooms modifiedBy');
        return {
          commitId: commit._id,
